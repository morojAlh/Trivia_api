...
def create_app(test_config=None):
  ...
  def search():
    ...
    try:
      ...
      searchResults = Question.query.order_by(Question.id).filter(Question.question.ilike('%' + search_term + '%')).all()
  ...
  def random_question():
    ...
    try: 
      ...
      for question in questions:
        try:
          index = previous_questions.index(question.id)
        except ValueError:
          index = -1

        if (index == -1):
          filterd_questions.append(question)

      current_question = False
      if len(filterd_questions) != 0:
        random_question = random.choice(filterd_questions)
        current_question = {
